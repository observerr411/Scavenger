...
export class ScavengerClient {
  ...
  private async invoke<T>(
    ...
    while (attempt < retries) {
      try {
        ...
        const result = await Promise.resolve({ result: { retval: nativeToScVal('MOCK', { type: 'string' }) } });
        
        // Map Soroban Error
        if ((result as any).error) {
          throw new ContractError((result as any).error, (result as any).errorCode);
        }
        ...
        if (attempt >= retries) {
          ...
          const errorMsg = error?.message || 'Unknown Contract Error';
  ...
  async initialize(admin: string, tokenAddress: string, charityAddress: string, collectorPercentage: number, ownerPercentage: number, signer: string) {
  ...
  async updateTokenAddress(admin: string, newAddress: string, signer: string) {
    return this.invoke<void>('update_token_address', [
      new Address(admin).toScVal(),
      new Address(newAddress).toScVal()
    ], signer);
  }

  async updateCharityAddress(admin: string, newAddress: string, signer: string) {
    return this.invoke<void>('update_charity_address', [
      new Address(admin).toScVal(),
      new Address(newAddress).toScVal()
    ], signer);
  }
  ...
  async updateCollectorPercentage(admin: string, newPercentage: number, signer: string) {
    return this.invoke<void>('update_collector_percentage', [
      new Address(admin).toScVal(),
      nativeToScVal(newPercentage, { type: 'u32' })
    ], signer);
  }

  async updateOwnerPercentage(admin: string, newPercentage: number, signer: string) {
    return this.invoke<void>('update_owner_percentage', [
      new Address(admin).toScVal(),
      nativeToScVal(newPercentage, { type: 'u32' })
    ], signer);
  }

  async updatePercentages(admin: string, collectorPercentage: number, ownerPercentage: number, signer: string) {
    return this.invoke<void>('update_percentages', [
      new Address(admin).toScVal(),
      nativeToScVal(collectorPercentage, { type: 'u32' }),
      nativeToScVal(ownerPercentage, { type: 'u32' })
    ], signer);
  ...
  async transferAdmin(currentAdmin: string, newAdmin: string, signer: string) {
    return this.invoke<void>('transfer_admin', [
      new Address(currentAdmin).toScVal(),
      new Address(newAdmin).toScVal()
    ], signer);
  }

  async registerParticipant(address: string, role: Role, name: string, latitude: number, longitude: number, signer: string): Promise<Participant> {
    return this.invoke<Participant>('register_participant', [
  ...
  async createIncentive(rewarder: string, wasteType: WasteType, rewardPoints: bigint, totalBudget: bigint, signer: string): Promise<Incentive> {
    return this.invoke<Incentive>('create_incentive', [
  ...
  async incentiveExists(incentiveId: number): Promise<boolean> {
    return this.invoke<boolean>('incentive_exists', [
      nativeToScVal(incentiveId, { type: 'u64' })
    ]);
  }

  async getIncentivesByRewarder(rewarder: string): Promise<number[]> {
    return this.invoke<number[]>('get_incentives_by_rewarder', [
      new Address(rewarder).toScVal()
    ]);
  }

  async getIncentivesByWasteType(wasteType: WasteType): Promise<number[]> {
    return this.invoke<number[]>('get_incentives_by_waste_type', [
      nativeToScVal(wasteType)
    ]);
  }
  ...
  async updateIncentive(incentiveId: number, newRewardPoints: bigint, newTotalBudget: bigint, signer: string): Promise<Incentive> {
    return this.invoke<Incentive>('update_incentive', [
      nativeToScVal(incentiveId, { type: 'u64' }),
  ...
  async deactivateIncentive(rewarder: string, incentiveId: number, signer: string): Promise<void> {
    return this.invoke<void>('deactivate_incentive', [
      new Address(rewarder).toScVal(),
      nativeToScVal(incentiveId, { type: 'u64' })
    ], signer);
  }

  async submitMaterial(submitter: string, wasteType: WasteType, weight: bigint, signer: string): Promise<Material> {
    return this.invoke<Material>('submit_material', [
  ...
  async deactivateWaste(admin: string, wasteId: number, signer: string): Promise<void> {
    return this.invoke<void>('deactivate_waste', [
      new Address(admin).toScVal(),
  ...
  async confirmWaste(wasteId: number, confirmer: string, signer: string): Promise<void> {
    return this.invoke<void>('confirm_waste', [
      nativeToScVal(wasteId, { type: 'u64' }),
  ...
  async resetWasteConfirmation(wasteId: number, owner: string, signer: string): Promise<void> {
    return this.invoke<void>('reset_waste_confirmation', [
      nativeToScVal(wasteId, { type: 'u64' }),
  ...
  async transferWaste(wasteId: number, from: string, to: string, signer: string): Promise<void> {
    return this.invoke<void>('transfer_waste', [
      nativeToScVal(wasteId, { type: 'u64' }),
  ...
  async distributeRewards(wasteId: number, incentiveId: number, manufacturer: string, signer: string): Promise<bigint> {
    return this.invoke<bigint>('distribute_rewards', [
      nativeToScVal(wasteId, { type: 'u64' }),
      nativeToScVal(incentiveId, { type: 'u64' }),
      new Address(manufacturer).toScVal()
    ], signer);
  ...
  async getParticipantStats(address: string): Promise<ParticipantStats> {
    return this.invoke<ParticipantStats>('get_participant_stats', [
      new Address(address).toScVal()
    ]);
  }
